...
const EXECUTION_PATTERNS = ['advisor', 'workflow']
...
function validateExecutionPolicy(policy) {
  ...
  if (policy.partitionable !== true)
    issues.push('execution.partitionable must be true for workflow mode')
  ...
  if (policy.writes === true && policy.isolation !== 'worktree')
...
function resolveExecutionPolicy(requested, defaults = {}) {
  ...
  if (!requested || requested.pattern !== 'workflow') {
  ...
  if (issues.length > 0) {
    return {
      ...advisorPolicy(configuredAdvisor, 'workflow request was incomplete or unsafe; using advisor mode'),
      fallbackIssues: issues,
    }
  }
  return {
    ...candidate,
    reason: requested.reason ?? 'task has explicit independent partitions and bounded verification',
  }
}
