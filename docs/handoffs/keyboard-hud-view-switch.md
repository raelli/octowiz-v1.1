...
**Status:** ready-for-agent — **blocked on a one-time, user-granted admin elevation**
...
3. **Isolate the opcode.** Diff the logs across switches to find the report that
   ...
   index byte and the usual `AA 55` / `55 AA` trailer. Candidate opcodes seen but
   unmapped in our notes: `04 13`, `04 F0` — verify, don't assume.
...
5. **Ship it.** Add `set_view(view)` / `show_slot(n)` to `driver.py` (mirroring the
   ...
   `apply.py` flips to ELLI on `Stop`/`SessionEnd` (when no other live session) and
   back to the card on the next screen event. Keep it inside the existing lock +
   change-detection so it doesn't fight concurrent updates.
...
- The HUD auto-switches to ELLI on idle/stop and back to the card on activity, with **no
  full re-upload** for the switch (the images already live in flash).
