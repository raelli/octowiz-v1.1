...
describe('daemon.processTask (forwarding)', () => {
  ...
  it('rejects octowiz.advise as unknown capability (removed; handled by AELLI)', async () => {
    ...
    expect(postResult).toHaveBeenCalledWith('t-advise', 'lt-advise', expect.objectContaining({ status: 'error', message: expect.stringContaining('unknown capability') }))
  })
  ...
  it('handles octowiz.observe capability and returns completed', async () => {
    ...
    expect(postResult).toHaveBeenCalledWith('t-obs', 'lt-obs', expect.objectContaining({
      status: 'completed',
  ...
  it('posts error when cwd is outside allowed roots', async () => {
    ...
    expect(postResult).toHaveBeenCalledWith('t3', 'lt-1', expect.objectContaining({ status: 'error' }))
  ...
  it('posts error when A2A server returns non-200', async () => {
    ...
    expect(postResult).toHaveBeenCalledWith('t4', 'lt-1', expect.objectContaining({ status: 'error' }))
    server.close()
  })
  ...
  it('normalizes A2A error artifact to queue error status', async () => {
    ...
    expect(postResult).toHaveBeenCalledWith('t6', 'lt-1', expect.objectContaining({ status: 'error', message: 'task is required' }))
    server.close()
  })
  ...
  it('[P1] aliases session_id to sessionId in dispatch result for queue consumers', async () => {
    ...
    expect(postResult).toHaveBeenCalledWith('t8', 'lt-1', expect.objectContaining({ session_id: 'sess-abc123', sessionId: 'sess-abc123' }))
    server.close()
  })
...
describe('daemon.processTask — router.validation-request', () => {
  ...
  it('passes valid JS draft — posts passed:true with workflowTaskId echoed', async () => {
    ...
    expect(postResult).toHaveBeenCalledWith('t-val-1', 'lt-val', expect.objectContaining({ status: 'completed', passed: true, workflowTaskId: 'wf-abc' }))
  })
  ...
  it('fails broken JS syntax — posts passed:false with failureKind:syntax-error', async () => {
    ...
    expect(postResult).toHaveBeenCalledWith('t-val-2', 'lt-val', expect.objectContaining({
      status: 'completed',
      passed: false,
  ...
  it('rejects advisory with unknown type', async () => {
    ...
    expect(postResult).toHaveBeenCalledWith('t-bad-type', 'lt-bad', expect.objectContaining({ status: 'error', failureKind: 'unknown-advisory-type' }))
  })
  ...
  it('rejects advisory with missing type', async () => {
    ...
    expect(postResult).toHaveBeenCalledWith('t-missing-type', 'lt-missing', expect.objectContaining({ status: 'error', failureKind: 'unknown-advisory-type' }))
  })
  ...
  it.each(['file-conflict', 'branch-drift', 'spec-deviation'])(
    ...
    async (type) => {
      ...
      expect(postResult).toHaveBeenCalledWith(`t-${type}`, 'lt-ok', expect.objectContaining({ status: 'completed', advisory }))
    },
  )
})
